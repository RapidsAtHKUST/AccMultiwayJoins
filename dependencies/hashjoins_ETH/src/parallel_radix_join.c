...
typedef struct LFTJ_arg_t  arg_t;
...
struct synctimer_t {
    ...
    uint64_t sync1[3]; /* for origin R and for origin S */
...
struct part_t {
    tuple_t *  origin;
    tuple_t *  tmp_hash_keys;
    int32_t ** hist;
    int64_t *  output;
    LFTJ_arg_t   *  thrargs;
    uint64_t   total_tuples;
    uint32_t   num_tuples;
    int32_t    first_pass_bits;
    uint32_t   second_pass_bits;
    int        relidx;  /* 0: R, 1: S */
    uint32_t   padding;
} __attribute__((aligned(CACHE_LINE_SIZE)));
...
prefetch(void * addr)
{
    ...
    /* _mm_prefetch(dir_addr, _MM_HINT_T0); */
    /* #endif */
}
...
{
    const tuple_t * restrict rel    = part->origin;
    ...
    const int32_t  R       = part->first_pass_bits;
    const int32_t  D       = part->second_pass_bits;
    const uint32_t fanOut  = 1 << D;
    ...
    tuple_t * restrict tmp = part->tmp_hash_keys;
...
{
    LFTJ_arg_t * args   = (LFTJ_arg_t*) param;
    ...
    part.first_pass_bits       = 0;
    part.second_pass_bits       = NUM_RADIX_BITS / NUM_PASSES; //PASS1RADIXBITS
    part.thrargs = args;
    part.padding = PADDING_TUPLES;

    /* 1. partitioning for relation R */
    part.origin          = args->relR;
    part.tmp_hash_keys          = args->tmpR;
    ...
    part.origin          = args->relS;
    part.tmp_hash_keys          = args->tmpS;
...
#warning Only 2-pass partitioning is implemented, set NUM_PASSES to 2!
    ...
    while(1) {
        ...
        /* 1. partitioning for relation R */
        part.origin          = (*args->skewtask)->relR.tuples + my_tid * numperthr;
        ...
        /* 2. partitioning for relation S */
        part.origin          = (*args->skewtask)->relS.tuples + my_tid * numperthr;
