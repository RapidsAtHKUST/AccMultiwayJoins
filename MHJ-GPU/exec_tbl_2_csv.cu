...
/*
 * Read selective attrs of a .tbl file from TPC-H benchmark and write to a .csv file with mmap
...
void tbl_2_csv(string tbl_addr, vector<AttrType> retrieve_attr_idxes, vector<AttrType> new_attr_idxes) {
    Timer t;
    FILE *fp, *fc;
    char *line = nullptr;
    size_t len = 0;
    ...
    string csv_addr = to_string(table_cardinality);
    for(auto i = 0; i < new_attr_idxes.size(); i++) {
        csv_addr = csv_addr + "_" + to_string(new_attr_idxes[i]);
    }
    csv_addr += csv_prefix;
    log_info("Write file %s", csv_addr.c_str());
    t.reset();
    write_rel_cols_csv(csv_addr.c_str(), retrieved_data, table_cardinality);
    myTime = t.elapsed();
    log_info("Finish writing the %s file", csv_prefix.c_str());
...
int main(int argc, char *argv[]) {
    ...
    tbl_2_csv<KeyType,CarType>(tbl_addr, retrieve_attr_idxes, new_attr_idxes);
    return 0;
}