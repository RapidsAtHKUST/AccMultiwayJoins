...
void test_general(string dir_name, bool ooc, int sbp) { //todo: all build tables have the same #buckets
    CUDAMemStat memstat;
    CUDATimeStat timing;
...
//    t_total.reset();
    ...
    gpu_time_idx = timing.get_idx();
    if (sbp == 0) {
        OOC<DataType,CntType, true, TYPE_AMHJ_OPEN_ADDR> ooc_wrapper(1.0*total_budget/1024.0/1024.0);
        ooc_wrapper.execute(relations[0], hash_tables, num_hash_tables,
                            used_for_compare, bucket_vec, num_attrs_in_res,
                            attr_idxes_in_iRes, num_attr_idxes_in_iRes,
                            ooc, res_gpu, &memstat, &timing);
    }
    else {
        OOC<DataType,CntType, true, TYPE_AMHJ_DRO_OPEN_ADDR> ooc_wrapper(1.0*total_budget/1024.0/1024.0);
        ooc_wrapper.execute(relations[0], hash_tables, num_hash_tables,
                            used_for_compare, bucket_vec, num_attrs_in_res,
                            attr_idxes_in_iRes, num_attr_idxes_in_iRes,
                            ooc, res_gpu, &memstat, &timing);
...
int main(int argc, char *argv[]) {
    ...
    test_general<KeyType,CarType>(string(argv[1]), (bool)stoi(argv[2]), stoi(argv[3]));
    return 0;
}