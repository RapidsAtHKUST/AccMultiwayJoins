...
//    t_total.reset();
    ...
    /*probe phase*/
    AMHJ_cuckoo_hashing<DataType,CntType,true> AMHJ_instance(num_attrs_in_res, relations[0].length, &memstat, &timing);

    gpu_time_idx = timing.get_idx();
    AMHJ_instance.AMHJ_evaluate(relations[0], hash_tables, num_hash_tables, used_for_compare, attr_idxes_in_iRes,
                               num_attr_idxes_in_iRes, res_gpu);
