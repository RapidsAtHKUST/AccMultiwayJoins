...
//            set<pair<DataType,DataType>> sort_set;
//            for(CntType i = 0; i < len; i++) {
//                sort_set.insert(make_pair(data_in[0][i], data_in[1][i]));
//            }
//            CntType inddd = 0;
//            for(auto elem : sort_set) {
//                data_in[0][inddd] = elem.first;
//                data_in[1][inddd] = elem.second;
//                inddd++;
//            }
...
#pragma omp parallel for
...
#else
    log_info("The data is already sorted. Igore sorting");
#endif
    ...
    if (numCol > 2) CUDA_MALLOC(&diff_bits_arr, sizeof(bool*)*(numCol-2), memstat);
...
//            CUBSelect(ascending_offsets, out_offset_temp, diff_bits_arr[c], len, memstat, timing);
    ...
    /*data_out[numCol-1] = data_in[numCol-1], hard copy*/
    ...
    CUDA_FREE(diff_bits_cnttype, memstat);
#endif
}
